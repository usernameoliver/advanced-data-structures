...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		Map<Integer, List<Integer>> degreeVertices = new HashMap<>();
		...
		for (int i = 0; i < numberOfVertices; i++) {
			...
			List<Integer> verticesOftheSameDegree = degreeVertices.getOrDefault(degree, new ArrayList<Integer>());
			verticesOftheSameDegree.add(i);
			degreeVertices.put(degree, verticesOftheSameDegree);	
		}
		for (int i = 2 * (numberOfVertices - 1); i >= 0; i--) {
			if (degreeVertices.containsKey(i)) {
				degreeSequence.addAll(degreeVertices.get(i));
			}
		}
